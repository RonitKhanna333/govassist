    ...
    <main className="presentation-page">
      <div className="presentation-shell">
        ...
        <section className="presentation-section journey" id="live-prototype">
          ...
          <div className="launch-card"><div><Status kind="deployed">Production deployment</Status><h3>Run the PMFME demonstration</h3><p>Use the production root for the signed-out PMFME flow. PR #5 is merged to <code>main</code>, and the completed Vercel deployment serves <code>/presentation</code> and <code>/evidence</code> as production routes.</p></div><ExternalLink href={LIVE_SITE}>Open production site <span aria-hidden="true">↗</span></ExternalLink></div>
        </section>
        ...
        <section className="presentation-section" id="evidence">
          ...
          <div className="evidence-grid"><article><h3>Repository and source</h3><p>Implementation, tests, corpus and diagrams:</p><ExternalLink href={REPOSITORY}>github.com/RonitKhanna333/govassist</ExternalLink><p className="small-copy">Official PMFME source:</p><ExternalLink href={SOURCE_PDF}>SchemeGuidelines.pdf</ExternalLink></article><article><h3>Verification commands</h3><pre>{verificationCommands}</pre><p>Run these in the repository root. CI passed on <code>main</code> after PR #5 merged.</p></article><article><h3>Browser proof</h3><p>Production-route verification remains separate from build evidence: desktop/mobile layout, eligible/ineligible flows, the exact age-18 boundary, scheme picker, microphone, TTS, language flows, console and link checks.</p><Status kind="planned">Manual production verification pending</Status></article></div>
        </section>
        ...
        <section className="presentation-section final-section" id="next">
          <p className="kicker">09 · NEXT</p><h2>Human review is the remaining gate.</h2>
          <div className="next-grid"><div><Status kind="limited">Human gate required</Status><p>Review PMFME clauses again, then confirm all eight conditions with special attention to the implemented boundary <code>profile.age &gt; 18</code>. The exact commands are on the evidence page.</p><a className="text-link" href="/evidence">Open remediation steps ↗</a></div><div><Status kind="limited">Human re-review pending</Status><p>PR #5 is merged to <code>main</code>; CI passed after the merge and Vercel production deployment completed. Deployment does not constitute human corpus approval.</p><ExternalLink href={PR_URL}>Open merged PR #5 ↗</ExternalLink></div></div>
        </section>
