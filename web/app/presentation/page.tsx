    ...
    <main className="presentation-page">
      <div className="presentation-shell">
        ...
        <section className="presentation-section journey" id="live-prototype">
          ...
          <div className="launch-card"><div><Status kind="deployed">Existing hosted prototype</Status><h3>Run the PMFME demonstration</h3><p>Use the live route for a signed-out demo. The presentation route is implemented here and needs an authorized deployment before it can be claimed as hosted.</p></div><ExternalLink href={LIVE_SITE}>Open hosted site <span aria-hidden="true">↗</span></ExternalLink></div>
        </section>
        ...
        <section className="presentation-section diagrams" id="uml">
          ...
          <p className="section-intro">These are rendered, responsive previews of the Mermaid sources in <code>docs/diagrams/</code>. They describe the current implementation, including optional paths and human-only review gates.</p>
        ...
        <section className="presentation-section final-section" id="next">
          ...
          <div className="next-grid"><div><Status kind="limited">Human gate required</Status><p>Review PMFME clauses again, then confirm all eight conditions with special attention to <code>age &gt; 18</code>. The exact commands are on the evidence page.</p><a className="text-link" href="/evidence">Open remediation steps ↗</a></div><div><Status kind="planned">Authorized deployment</Status><p>After review, push the commit and redeploy the API and web projects. Then verify <code>/presentation</code> and repeat the signed-out browser evidence.</p><ExternalLink href={REPOSITORY}>Open deployment source ↗</ExternalLink></div></div>
        </section>
